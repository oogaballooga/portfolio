...
export default function Page() {
  ...
  return (
    <main onClick={() => setActiveCard(null)} className="relative min-h-screen text-white">
      {/* Background */}
      <div
      ...
      {/* Masters Degree */}
      <div className='
        border-5 border-[rgb(100,100,100)] rounded-3xl p-5 bg-black
        absolute top-[37%] left-[20%] translate-x-[-50%] translate-y-[-50%] '
      >
      ...
      <div className='
        border-5 border-[rgb(100,100,100)] rounded-3xl p-5 bg-black
        absolute top-[67%] left-[20%] translate-x-[-50%] translate-y-[-50%] '
      >
      ...
      <div className="grid-wrapper absolute top-[22%] left-[67%] translate-x-[-50%] ">
        <div className="course-grid">
          {courses.map((course) => {
            const isActive = activeCard === course.id;
            return (
            <CourseCard
              key={course.id}
              {...course}
              style={{
                ...
                zIndex: cardZ[course.id] ?? 100,
              }}
              activeWidth={course.activeWidth}
              isActive={isActive}
              onActivate={() => {
                if (activeCard === course.id){
                  setActiveCard(null);
                }
                else{
                  setCurrZIndex(prev => {
                    const next = prev + 1;
                    setCardZ(z => ({ ...z, [course.id]: next }));
                    return next;
                  });
